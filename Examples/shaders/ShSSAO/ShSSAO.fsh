...
// This line is also present in the SsaoInit script, so don't forget to
// comment out that one as well!
#define SSAO_WORLD_SPACE_NORMALS

// Must be the same values as in the SsaoInit script!
#define SSAO_KERNEL_SIZE 16

varying vec2 v_vTexCoord;

#define u_texDepth gm_BaseTexture
uniform sampler2D u_texNormal;
uniform sampler2D u_texRandom;
#ifdef SSAO_WORLD_SPACE_NORMALS
...
uniform vec3 u_vSampleKernel[SSAO_KERNEL_SIZE]; // Kernel of random vectors.
uniform vec2 u_vNoiseScale;                     // (screenWidth, screenHeight) / SSAO_NOISE_TEXTURE_SIZE
...
void main()
{
...
#endif
	vec3 random = texture2D(u_texRandom, v_vTexCoord * u_vNoiseScale).xyz * 2.0 - 1.0;
	...
	{
		// Get a sample in view-space and get its screen-space coordinates
	...
	occlusion = clamp(1.0 - occlusion / float(SSAO_KERNEL_SIZE), 0.0, 1.0);
