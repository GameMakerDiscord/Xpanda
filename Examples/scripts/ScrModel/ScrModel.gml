...
function Model() constructor
{
	...
	/// @func load_buffer(_path)
	/// @desc Loads a model from a binary file.
	/// @param {string} _path Path to the file.
	/// @return {Model} Returns `self`.
	/// @see Model.save_buffer
	static load_buffer = function (_path) {
	...
	/// @return {Model} Returns `self`.
	static load_obj = function (_path) {
		destroy();
		...
		while (!file_text_eof(_file))
		{
			...
			var _exploded = ce_string_explode(_line, " ");
			...
			case "f":
				...
				repeat (3)
				{
					var _face = ce_string_explode(_exploded[i++], "/");
